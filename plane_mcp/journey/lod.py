...
def _apply_lod_to_dict(data: Dict[str, Any], profile: LODProfile, project_identifier: Optional[str] = None) -> Dict[str, Any]:
    ...
    if profile == LODProfile.SUMMARY:
        ...
        # Special case: lift state name if state wasn't simple
        ...
        elif "state" not in result and "state_detail" in data and isinstance(data["state_detail"], dict):
            result["state"] = data["state_detail"].get("name")
            
        # Ensure ticket_id makes it to summary if injected
        if "ticket_id" in data and "ticket_id" in SUMMARY_FIELDS:
