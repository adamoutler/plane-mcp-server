...
def register_ai_journeys(mcp: FastMCP) -> None:
    ...
    def update_ticket(
        ...
        # Remove Nones to only send updated fields
    ...
    def begin_work(ticket_id: str) -> dict:
        ...
        existing_assignees = [a.get('id') for a in existing_item.assignees] if hasattr(existing_item, 'assignees') and existing_item.assignees else []
    ...
    def search_tickets(query: str, project_slug: str = "PLANE") -> list[dict]:
        ...
        try:
            project_id = resolver.get_project_id(project_slug)
            # Plane's search is workspace-wide but we can filter or just search globally
            results = client.work_items.search(workspace_slug=workspace_slug, query=query, params=RetrieveQueryParams())
            
            minified = []
            # results is a WorkItemSearch object
